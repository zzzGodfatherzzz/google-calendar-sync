 ...
 * - Push Bookings table entries to Google Calendar
 * - Auto-generate Google Meet links if empty
 * - All config prompted at install
 *
 * Notes for easy modification:
 * 1️⃣ Booking table fields can be changed in the BOOKING_FIELDS section
 * 2️⃣ User table fields can be changed in the USER_FIELDS section
 * 3️⃣ BASE_URL should be updated to your domain
 * 4️⃣ OAuth scopes or Meet behavior can be adjusted in the OAUTH / EVENT section
 */
...
const BASE_URL = "https://business-system.app"; // <-- update if needed
...
const BOOKING_FIELDS = {
  ...
  guest_email2: "guest_email2",
  guest_email3: "guest_email3",
  guest_email4: "guest_email4",
...
async function updateBookingEvent(id, eventId, meetLink) {
  await db.query(
    `UPDATE "Bookings" SET ${BOOKING_FIELDS.google_event_id}=$1, ${BOOKING_FIELDS.meeting_link}=$2 WHERE ${BOOKING_FIELDS.id}=$3`,
    [eventId, meetLink || "", id]
  );
}
...
async function saveUserTokens(userId, refreshToken, calendarId = "primary") {
  await db.query(
    `UPDATE users SET ${USER_FIELDS.refresh_token}=$1, ${USER_FIELDS.calendar_id}=$2 WHERE id=$3`,
...
async function createGoogleEvent(user, booking) {
  ...
  const eventResource = {
    summary: booking[BOOKING_FIELDS.guest_name]
      ? `Meeting with ${booking[BOOKING_FIELDS.guest_name]}`
      : "Scheduled Meeting",
  ...
  await updateBookingEvent(booking[BOOKING_FIELDS.id], event.id, meetLink);
  return event;
}
...
function routes() {
  ...
  router.post("/bookings/push", async (req, res) => {
    try {
      ...
      const user = await loadUserById(booking[BOOKING_FIELDS.host_user_id]);
  ...
  // === Return routes in iterable array ===
  return [
    {
